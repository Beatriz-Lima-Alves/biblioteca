...
class Usuarios extends Controller{
    ...
    public function validar()
    {
        ...
        if (empty($usuario) || empty($clave)) {
            ...
            if ($data) {
                ...
                $msg = array('msg' => 'Usuário ou senha incorretos', 'icono' => 'warning');
            }
        }
    ...
    public function eliminar(int $id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Usuario desativado', 'icono' => 'success');
        }else{
        ...
        if (!$perm && $id_user != 1) {
            echo '<div class="card">
                    <div class="card-body text-center">
                        <span class="badge badge-danger">Não tem permissão para acessar essa área do sistema</span>
                    </div>
                </div>';
            exit;
        }
    ...
    public function cambiarPas()
    {
        if ($_POST) {
            ...
            if (hash("SHA256", $clave) == $user['clave']) {
                ...
                if ($data == "modificado") {
                    $msg = array('msg' => 'Senha modificada', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Erro ao modificar', 'icono' => 'warning');
                }
            } else {
                $msg = array('msg' => 'Senha atual está incorreta', 'icono' => 'warning');
            }
            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
            die();
        }
