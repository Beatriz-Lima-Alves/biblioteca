...
class Configuracion extends Controller
{
    ...
    public function index()
    {
        ...
        $perm = $this->model->verificarPermisos($id_user, "Configuração");
        if (!$perm && $id_user != 1) {
    ...
    public function actualizar()
    {
        ...
        if (empty($id) || empty($nombre) || empty($telefono) || empty($direccion) || empty($correo)) {
            ...
            if ($data == "modificado") {
                $msg = array('msg' => 'Dados da empresa modificados', 'icono' => 'success');
                if (!empty($img['name'])) {
    ...
    public function libros()
    {
        ...
        $pdf->Cell(50, 5, utf8_decode('Estudante'), 1, 0, 'L');
        $pdf->Cell(87, 5, 'Livro', 1, 0, 'L');
        $pdf->Cell(30, 5, 'Emprestimo feito em', 1, 0, 'L');
