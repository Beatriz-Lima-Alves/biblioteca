...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const language = {
        "decimal": "",
        "emptyTable": "Não tem informações",
...
function frmUsuario() {
    document.getElementById("title").textContent = "Novo Usuario";
    document.getElementById("btnAccion").textContent = "Salvar";
...
function registrarUser(e) {
    ...
    if (usuario.value == "" || nombre.value == "") {
        alertas('Todos os campos obrigatórios devem ser preenchidos', 'warning');
    } else {
...
function btnEditarUser(id) {
    document.getElementById("title").textContent = "Atualizar";
...
function btnEliminarUser(id) {
    Swal.fire({
        title: 'Certeza que deseja desativar este usuário?',
...
function btnReingresarUser(id) {
    Swal.fire({
        title: 'Deseja ativar o registro novamente?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function btnEliminarEst(id) {
    Swal.fire({
        ...
        text: "O aluno não será excluído permanentemente, apenas mudará o status para inativo!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function registrarMateria(e) {
    ...
    if (materia.value == "") {
        alertas('O nome da matéria é obrigatório', 'warning');
    } else {
...
function btnEliminarMat(id) {
    Swal.fire({
        title: 'Certeza que deseja desativar essa matéria',
...
function btnEliminarAutor(id) {
    Swal.fire({
        title: 'Você tem certeza que quer desativar?',
        text: "O Autor não será excluído permanentemente, apenas alterará o status para inativo!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function btnEliminarEdi(id) {
    Swal.fire({
        title: 'Você tem certeza que quer desativar?',
        text: "A editora não será excluída permanentemente, apenas mudará o status para inativo!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function btnEliminarLibro(id) {
    Swal.fire({
        title: 'Você tem certeza que quer desativar?',
        text: "O livro não será excluído permanentemente, apenas mudará o status para inativo!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
...
function preview(e) {
    ...
    if (!extension.exec(filePath)) {
        alertas('Selecione um arquivo válido', 'warning');
        deleteImg();
        return false;
    }else{
...
function modificarClave(e) {
    ...
    formClave.onsubmit = function (e) {
        ...
        if (clave_actual == "" || nueva_clave == "" || confirmar_clave == "") {
            ...
            alertas('As senhas não correspondem', 'warning');
        } else {
            const http = new XMLHttpRequest();
            const frm = document.getElementById("frmPermisos");
...
function verificarLibro(e) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            const res = JSON.parse(this.responseText);
            if (res.icono == 'success') {
                document.getElementById('msg_error').innerHTML = `<span class="badge badge-primary">Disponivel: ${res.cantidad}</span>`;
            }else{
                alertas(res.msg, res.icono);
                return false;
            }
